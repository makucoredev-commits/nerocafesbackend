  ...
  return async (req, res, next) => {
    ...
    res.json = function(data) {
      ...
      logAudit(req, action, data).catch(err => {
        logger.error('SECURITY', `AuditLog: Failed to log action ${action}: ${err.message}`, { error: err });
      });
...
async function logAudit(req, action, responseData) {
  try {
    ...
    logger.error('SECURITY', `AuditLog: Failed to create audit log entry: ${error.message}`, { error });
  }
}
...
export async function getAuditLogs(filters = {}, limit = 100) {
  try {
    ...
    logger.error('SECURITY', `AuditLog: Failed to get audit logs: ${error.message}`, { error });
    return [];
  }
}
