...
/**
 * Synchronizes a single User into the Customer CRM collection.
...
export async function syncUserToCustomer(user) {
  ...
  try {
    ...
    // 1. Find existing customer by userId
    ...
    if (!customer && normPhone && !normPhone.startsWith('user_')) {
      customer = await Customer.findOne({ phone: normPhone });
    }
    ...
    // 3. Create a new Customer entry linked to this User
    ...
    logger.success('ADMIN', `Auto-synced new Customer from User account: ${user.name} (${user.email || user.phone})`);
    return customer;
  } catch (err) {
    logger.warn('ADMIN', `Failed to sync user to customer: ${err.message}`, { error: err });
    return null;
  }
}

/**
 * Synchronizes all User documents in the database to Customer documents.
 ...
 */
export async function syncAllUsersToCustomers() {
  try {
    ...
    for (const u of users) {
      const result = await syncUserToCustomer(u);
      if (result) count++;
    }
    return count;
  } catch (err) {
    logger.warn('ADMIN', `Batch user-to-customer sync error: ${err.message}`);
    return 0;
  }
}
