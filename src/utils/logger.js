...
class Logger {
  log(level, module, message, meta = {}) {
    ...
    const useJsonFormat = (isProduction && !isTTY) || forceJson;
    ...
    if (useJsonFormat) {
      ...
      // Development/TTY format: [Timestamp] [RequestID] [Module] [Level] Message
      const levelColor = LOG_LEVELS[level]?.color || '';
      const reqIdPart = requestId === 'system' ? 'system' : requestId;
      
      let formattedLog = `[${timestamp}] [${reqIdPart}] [${module}] ${levelColor}${level}${RESET_COLOR} ${message}`;
