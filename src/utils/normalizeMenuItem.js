...
export const normalizeMenuItem = (item) => {
  if (!item || typeof item !== 'object') {
    logger.warn('INVENTORY', 'normalizeMenuItem: Invalid item received', { item });
    return null;
  }
  ...
  try {
    ...
    logger.error('INVENTORY', `normalizeMenuItem: Error normalizing item ${item?._id}: ${error.message}`, { itemId: item?._id, error });
...
export const normalizeMenuItems = (items) => {
  if (!Array.isArray(items)) {
    logger.warn('INVENTORY', 'normalizeMenuItems: Invalid items received, expected array', { items });
    return [];
  }
  ...
  for (const item of items) {
    try {
      ...
      logger.error('INVENTORY', `normalizeMenuItems: Failed to normalize item ${item?._id}: ${error.message}`, { itemId: item?._id, error });
    }
  }

  if (errors.length > 0) {
    logger.warn('INVENTORY', `normalizeMenuItems: ${errors.length} items failed to normalize. Continuing with ${normalized.length} valid items.`);
  }
