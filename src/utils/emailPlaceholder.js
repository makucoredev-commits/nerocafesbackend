...
export async function sendWelcomeEmail({ to, name, email, password }) {
  console.log('[Email] welcome → sending to:', to, name);
  ...
  try {
    ...
    console.warn('[Email] Failed to create verification token:', err.message || err);
  ...
  if (!transporter) {
    console.error('[Email] Welcome email SMTP not configured.');
  ...
  try {
    ...
    console.log(`[Email] Welcome email sent in ${duration}ms to:`, to);
    return { ok: true, info };
  } catch (err) {
...
export async function sendOrderConfirmationEmail({ to, name, orderNo, total, items = [], orderId }) {
  console.log('[Email] order confirmation → sending to:', to, name);
  const transporter = getTransporter();
  const fromUser = process.env.SMTP_USER || 'nerocafes14@gmail.com';
  const clientOrigin = process.env.CLIENT_ORIGIN || 'http://localhost:5173';
  ...
  if (!transporter) {
    console.error('[Email] Order confirmation SMTP not configured.');
    return { ok: false, error: 'SMTP not configured' };
  }
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    console.log(`[Email] Confirmation email sent in ${duration}ms to:`, to);
    return { ok: true, info };
  } catch (err) {
...
export async function sendInvoiceEmail({ to, name, orderId, orderNo, total, items = [], discountAmount = 0 }) {
  console.log('[Email] invoice → sending to:', to, name);
  const transporter = getTransporter();
  const fromUser = process.env.SMTP_USER || 'nerocafes14@gmail.com';
  const clientOrigin = process.env.CLIENT_ORIGIN || 'http://localhost:5173';
  ...
  if (!transporter) {
    console.error('[Email] Invoice SMTP not configured.');
    return { ok: false, error: 'SMTP not configured' };
  }
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    console.log(`[Email] Invoice email sent in ${duration}ms to:`, to);
    return { ok: true, info };
  } catch (err) {
...
export async function sendOrderStatusEmail({ to, name, orderNo, status, orderId, reason }) {
  console.log('[Email] status update → sending to:', to, name, status);
  const transporter = getTransporter();
  const fromUser = process.env.SMTP_USER || 'nerocafes14@gmail.com';
  const clientOrigin = process.env.CLIENT_ORIGIN || 'http://localhost:5173';
  ...
  if (!transporter) {
    console.error('[Email] Order status update SMTP not configured.');
    return { ok: false, error: 'SMTP not configured' };
  }
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    console.log(`[Email] Status update email sent in ${duration}ms to:`, to);
    return { ok: true, info };
  } catch (err) {
...
export async function sendPasswordResetEmail({ to, name, resetLink }) {
  console.log('[Email] password reset → sending to:', to, name);
  const transporter = getTransporter();
  const fromUser = process.env.SMTP_USER || 'nerocafes14@gmail.com';
  ...
  if (!transporter) {
    console.error('[Email] Password reset SMTP not configured.');
    return { ok: false, error: 'SMTP not configured' };
  }
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    console.log(`[Email] Password reset email sent in ${duration}ms to:`, to);
    return { ok: true, info };
  } catch (err) {
