...
export async function sendOtpEmail({ to, name, otp }) {
  logger.info('EMAIL', `Email Sending: login OTP → ${to}`);
  const transporter = getTransporter();
  const fromUser = process.env.SMTP_USER || 'nerocafes14@gmail.com';

  const html = buildEmailTemplate('Your Login Code', name, `
    <p>Use the following 6-digit verification code to log in to your NeroCafes account:</p>

    <div style="text-align: center; margin: 24px 0;">
      <div style="display: inline-block; background: rgba(201, 169, 98, 0.15); border: 2px solid #c9a962; border-radius: 12px; padding: 16px 32px;">
        <span style="font-size: 32px; font-weight: 800; letter-spacing: 8px; color: #FFFDD0; font-family: monospace;">${otp}</span>
      </div>
    </div>

    <div class="info-box">
      <p style="margin: 0; color: #c9a962; font-weight: bold;">⏱️ Valid for 10 minutes</p>
      <p style="margin: 6px 0 0; font-size: 13px; opacity: 0.7;">Never share this verification code with anyone. NeroCafes staff will never ask for your code.</p>
    </div>

    <p class="muted" style="margin-top: 18px;">If you did not request this code, you can safely ignore this email.</p>
  `);

  if (!transporter) {
    logger.warn('EMAIL', `SMTP not configured — Login OTP for ${to}: ${otp}`);
  ...
  try {
    ...
    const info = await transporter.sendMail({
      from: `"NeroCafes" <${fromUser}>`,
      to,
      subject: `Your NeroCafes Login Code: ${otp} ☕`,
      html,
    });
    const duration = Date.now() - startTime;
    logger.success('EMAIL', `Email Sent: Login OTP email in ${duration}ms to ${to}`, { duration });
    return { ok: true, info };
  } catch (err) {
    logger.error('EMAIL', `Email Failed: Login OTP email to ${to}: ${err.message}`, { error: err });
    return { ok: false, error: err.message, devMode: true, otp };
  }
}
