...
export async function upsertCustomer({ name, phone, email, userId, totalSpent = 0, branchId = 'branch_001' }) {
  logger.debug('ORDERS', 'Customer Loading/Upserting...', { phone, branchId });
  const normPhone = normalizePhone(phone);
  let BranchCustomer;
  try {
    BranchCustomer = await branchDbManager.getBranchModel(branchId, 'Customer');
  } catch (err) {
    logger.warn('ORDERS', `Failed to get Customer model for branch ${branchId}, using default: ${err.message}`);
    BranchCustomer = Customer;
  }
  let c = await BranchCustomer.findOne({ phone: normPhone });
  if (c) {
    c.name = name;
...
export async function createOrderFromBody({ items, customer, paymentMethod, isOutOfRange, userId, location, io, branchId = 'branch_001' }) {
  logger.info('ORDERS', `Order Request Received for branch: ${branchId}`, { userId, paymentMethod, branchId });
  logger.debug('ORDERS', 'Order Validation Started');

  let MenuItemModel = MenuItem;
  let InventoryItemModel = null;
  let StockMovementModel = StockMovement;
  try {
    MenuItemModel = await branchDbManager.getBranchModel(branchId, 'MenuItem');
    InventoryItemModel = await branchDbManager.getBranchModel(branchId, 'InventoryItem');
    StockMovementModel = await branchDbManager.getBranchModel(branchId, 'StockMovement');
  } catch (err) {
    logger.warn('ORDERS', `Failed to get branch models for branch ${branchId}, using default: ${err.message}`);
  }
  ...
  for (const line of normalizedItems) {
    const menu = await MenuItemModel.findById(line.menuItemId).populate('ingredients.inventoryItemId');
    if (!menu) {
      logger.warn('ORDERS', `Item missing from database: ${line.menuItemId}`);
      throw new Error('An item in your order no longer exists');
    }
    if (menu.available === false) {
      logger.warn('ORDERS', `Item out of stock: ${menu.name}`);
      throw new Error(`${menu.name} is currently out of stock`);
    }
  ...
  for (const li of lineItems) {
    await MenuItemModel.updateOne({ _id: li.menuItemId }, { $inc: { orderCount: li.quantity } });
  }

  // Deduct inventory items stock
  logger.debug('ORDERS', 'Stock Reserved (Deducting current stock)');
  for (const li of lineItems) {
    const menu = await MenuItemModel.findById(li.menuItemId).populate('ingredients.inventoryItemId');
    if (!menu || menu.autoInventory !== true) continue;

    for (const ingredient of menu?.ingredients || []) {
      if (!ingredient.inventoryItemId) continue;
      const invId = ingredient.inventoryItemId._id || ingredient.inventoryItemId;
      let inventoryItem = null;
      if (InventoryItemModel) {
        inventoryItem = await InventoryItemModel.findById(invId);
      }
      ...
      await broadcastInventoryStockChange(io, inventoryItem._id, branchId);
    }
  }
  ...
  // Emit detailed order info to branch admins & kitchen (Strict Branch Isolation)
  logger.info('ORDERS', `Socket Broadcast: Kitchen & Admin Notified for branch: ${branchId}`);
  const orderPayload = {
    type: 'created',
    status: 'Received',
    orderId: order._id,
  ...
  io?.to(`branch:${branchId}:kitchen`).emit('orders:update', orderPayload);
  io?.to(`branch:${branchId}:admin`).emit('orders:update', orderPayload);
  io?.to('command-center').emit('orders:update', orderPayload);
  // Also emit to legacy room for default branch to ensure zero breaking changes
  if (branchId === 'branch_001') {
    io?.to('kitchen').emit('orders:update', orderPayload);
    io?.to('admin').emit('orders:update', orderPayload);
  }
