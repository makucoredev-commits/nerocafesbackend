...
router.get('/overview', authAdmin, async (req, res) => {
  try {
    ...
    const [
      ...
      Order.aggregate([
        { $match: { createdAt: { $gte: effectiveStartDate }, cancelledAt: null } },
        {
          $group: {
            _id: '$status',
            count: { $sum: 1 },
...
router.get('/devices', authAdmin, async (req, res) => {
  try {
    ...
    blockedList.forEach((b) => {
      if (b.target) blockedSet.add(b.target);
      if (b.ip) blockedSet.add(b.ip);
    ...
    const formattedSessions = sessions.map((s) => {
      ...
      const isBlocked = blockedSet.has(s.ip) || blockedSet.has(s.sessionId);
      let status = 'offline';
      if (isBlocked) {
        status = 'blocked';
      } else if (s.isRevoked) {
...
router.post('/blocked-devices/block', authAdmin, async (req, res) => {
  try {
    const { target, type = 'IP', ip, fingerprint, label = 'Blocked Device', reason = 'Blocked by administrator', deviceModel, browser, os } = req.body;
    const finalTarget = (target || ip || fingerprint || '').trim();

    if (!finalTarget) {
      return res.status(400).json({ error: 'Target IP, device fingerprint, or identifier required to block' });
    }
    ...
    const blockedDoc = await BlockedDevice.findOneAndUpdate(
      { target: finalTarget },
      {
        $set: {
          ...
          ip: ip || (type === 'IP' ? finalTarget : ''),
          fingerprint: fingerprint || (type === 'FINGERPRINT' ? finalTarget : ''),
    ...
    // Force revoke any active sessions associated with this IP or session
    if (ip || finalTarget) {
      await SessionPresence.updateMany(
        { $or: [{ ip: ip || finalTarget }, { sessionId: finalTarget }] },
        { $set: { isRevoked: true } }
      );
    }
    ...
    await AuditLog.create({
      ...
      target: `Blocked Device: ${finalTarget} (${label})`,
      ip: req.ip,
      ...
      status: 200,
      success: true,
      body: { target: finalTarget, label, reason, blockedBy: req.admin.email },
      timestamp: new Date(),
    });

    const io = req.app.get('io');
    if (io) {
      io.to('command-center').emit('device:blocked', { target: finalTarget });
    }
...
router.post('/blocked-devices/unblock', authAdmin, async (req, res) => {
  try {
    ...
    const io = req.app.get('io');
    if (io) {
      io.to('command-center').emit('device:unblocked', { target: unblockedDoc.target });
    }
