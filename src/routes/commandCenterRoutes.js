...
// In-memory tracker for failed admin login attempts
...
export function recordFailedAdminLogin(email, ip, reason = 'Invalid credentials') {
  ...
  failedLoginTracker.attempts.unshift(failure);
  if (failedLoginTracker.attempts.length > 200) {
    failedLoginTracker.attempts = failedLoginTracker.attempts.slice(0, 200);
  }
  failedLoginTracker.recentFailures++;
  ...
  AuditLog.create({
    ...
    body: { email, reason },
    timestamp: new Date(),
  }).catch((err) => logger.error('SECURITY', `Failed to audit failed login: ${err.message}`));
}
...
router.get('/overview', authAdmin, async (req, res) => {
  try {
    ...
    return res.json({
      ...
      orders: orderStats,
      security: {
        failedLoginAttemptsCount: failedLoginTracker.attempts.length,
...
router.get('/security', authAdmin, async (req, res) => {
  try {
    ...
    return res.json({
      ok: true,
      failedAttempts: failedLoginTracker.attempts.slice(0, 30),
      failedAttemptsCount: failedLoginTracker.attempts.length,
      activeAdminSessions: activeRefreshTokens,
      admins: adminsList,
      recentSecurityEvents: recentSecurityLogs,
      lastResetAt: failedLoginTracker.lastResetAt,
    });
  } catch (error) {
...
/**
 * Reset Admin Login Attempts Action
 */
router.post('/security/reset-admin-attempts', authAdmin, async (req, res) => {
  try {
    const previousCount = failedLoginTracker.attempts.length;
    failedLoginTracker.attempts = [];
    failedLoginTracker.recentFailures = 0;
    failedLoginTracker.lastResetAt = new Date();
    ...
    await AuditLog.create({
      ...
      userId: req.admin._id,
      actorName: req.admin.name || req.admin.email,
      target: 'Admin Auth Gateway Lockout Engine',
      ip: req.ip,
      userAgent: req.headers['user-agent'] || '',
      path: req.originalUrl,
      method: 'POST',
    ...
    return res.json({
      ok: true,
      message: `Successfully cleared ${previousCount} failed login attempt records. Rate limit locks refreshed.`,
