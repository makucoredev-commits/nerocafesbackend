...
router.post('/blocked-devices/block', authAdmin, async (req, res) => {
  try {
    ...
    const blockedDoc = await BlockedDevice.findOneAndUpdate(
      { target: finalTarget },
      {
        $set: {
          ...
          browser: browser || '',
          os: os || '',
          reason: reason || 'Violation of terms / Blocked by administrator',
    ...
    // Force revoke any active sessions associated with this IP, phone, or session
    const revokeOr = [{ target: finalTarget }, { ip: ip || finalTarget }, { sessionId: finalTarget }];
    ...
    await AuditLog.create({
      ...
      status: 200,
      success: true,
      body: { target: finalTarget, type, phone: cleanPhone, label, reason, blockedBy: req.admin.email },
      timestamp: new Date(),
    });
    ...
    if (io) {
      const blockPayload = {
        ...
        message: `You have been blocked from using NeroCafes. Reason: ${blockedDoc.reason}`,
        timestamp: new Date().toISOString(),
      };

      // Broadcast globally and to targeted rooms for immediate instant lockout
      io.emit('device:blocked', blockPayload);
      if (cleanPhone) {
