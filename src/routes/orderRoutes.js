...
router.post('/', authUser, requireUser, async (req, res) => {
  try {
    ...
    // Check if customer phone or IP is blocked
    const blockQueries = [{ target: ip }, { ip }];
    if (normPhone) {
    ...
    if (isBlocked) {
      return res.status(403).json({
        error: `Access Denied: You are blocked from placing orders. Reason: ${isBlocked.reason || 'Blocked by administrator'}`,
        blocked: true,
