...
router.post('/validate-cart', async (req, res) => {
  try {
    ...
    const MenuItemModel = await req.getBranchModel('MenuItem');
    for (const item of items) {
      const menuItemId = item.menuItemId || item.id || item._id;
      if (!menuItemId) continue;

      const menuItem = await MenuItemModel.findById(menuItemId).populate('ingredients.inventoryItemId');
...
router.post('/', authUser, requireUser, async (req, res) => {
  try {
    ...
    // Check if customer phone or specific device session is blocked (never block shared Wi-Fi IP)
    const blockQueries = [];
    if (clientSessionId) {
      blockQueries.push({ target: clientSessionId }, { sessionId: clientSessionId });
    }
    if (normPhone) {
      blockQueries.push({ target: normPhone }, { phone: normPhone });
      const raw10 = normPhone.startsWith('91') && normPhone.length === 12 ? normPhone.slice(2) : normPhone;
      blockQueries.push({ target: raw10 }, { phone: raw10 });
    }
    ...
    if (blockQueries.length > 0) {
      ...
      if (isBlocked) {
        return res.status(403).json({
          error: isBlocked.reason || defaultBlockedMsg,
          blocked: true,
...
router.get('/me', authUser, requireUser, async (req, res) => {
  try {
    const userPhone = req.user.phone ? normalizePhone(req.user.phone) : '';
    const rawDigits = req.user.phone ? String(req.user.phone).replace(/\D/g, '') : '';
    const userEmail = req.user.email ? String(req.user.email).trim().toLowerCase() : '';

    const phoneConditions = [];
    if (userPhone) {
      phoneConditions.push({ 'customer.phone': userPhone });
      phoneConditions.push({ 'customer.phone': `+${userPhone}` });
    }
    if (rawDigits) {
      phoneConditions.push({ 'customer.phone': rawDigits });
      if (rawDigits.length === 10) phoneConditions.push({ 'customer.phone': `91${rawDigits}` });
      if (rawDigits.startsWith('91') && rawDigits.length === 12) phoneConditions.push({ 'customer.phone': rawDigits.slice(2) });
    }
    ...
    const query = {
      $or: [
        { userId: req.user._id },
        ...phoneConditions,
        ...(userEmail && !userEmail.endsWith('@customer.nerocafe.com') ? [{ 'customer.email': userEmail }] : [])
      ]
    };
...
// Get tracking info for a user's specific order (by ID, user must own the order or have matching phone/email)
