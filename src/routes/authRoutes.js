...
async function checkBlockStatus(req, res, next) {
  try {
    ...
    const rawPhone = String(req.body?.phone || req.body?.identifier || '').replace(/\D/g, '');
    const cleanPhone = rawPhone.length === 12 && rawPhone.startsWith('91') ? rawPhone.slice(2) : rawPhone;

    const queries = [{ target: ip }, { ip }];
    if (cleanPhone) {
      queries.push({ target: cleanPhone }, { phone: cleanPhone });
      queries.push({ target: `91${cleanPhone}` }, { phone: `91${cleanPhone}` });
    }
    ...
    if (isBlocked) {
      return res.status(403).json({
        error: `Access Denied: You are blocked from using NeroCafes. Reason: ${isBlocked.reason || 'Blocked by administrator'}`,
        blocked: true,
...
router.use(['/signup', '/send-otp', '/verify-otp', '/login', '/google'], checkBlockStatus);
...
router.post('/signup', async (req, res) => {
  try {
    ...
    const branchId = req.headers['x-branch-id'] || 'branch_001';
    syncUserToCustomer(user, branchId).catch(err =>
