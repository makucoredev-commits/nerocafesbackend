...
router.post('/login', authLimiter, async (req, res) => {
  try {
    ...
    const isBlocked = await BlockedDevice.findOne({
      isActive: true,
      $or: [{ target: ip }, { ip }, { fingerprint: fp }, { target: fp }],
    });
    if (isBlocked) {
...
router.get('/customers', authAdmin, async (req, res) => {
  try {
    ...
    syncAllUsersToCustomers().catch(err =>
...
/* Manually trigger full sync of all registered users → Customer CRM records */
router.post('/customers/sync-users', authAdmin, async (req, res) => {
  try {
    ...
    logger.success('ADMIN', `Synced ${synced} users to Customer CRM (${skipped} skipped/errors)`);
...
router.post('/customers', authAdmin, async (req, res) => {
  try {
    ...
    // Check if a Customer CRM record already exists by phone
    let existingCustomer = await Customer.findOne({ phone: normPhone });
    ...
    if (existingUser) {
      const byUserId = await Customer.findOne({ userId: existingUser._id });
      if (byUserId) {
