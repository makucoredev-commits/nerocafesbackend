...
router.post('/orders', authAdmin, async (req, res) => {
  try {
    ...
    if (!existingUser) {
      logger.info('ADMIN', 'User lookup by normalized phone failed, trying raw phone', {
        normPhone,
      ...
      if (existingUser) {
        ...
        logger.info('ADMIN', 'User found via fallback normalization', {
    ...
    if (!existingUser && customer.email) {
      logger.info('ADMIN', 'Auto-creating new user account for customer', {
        name: customer.name,
        phone: normPhone,
        email: customer.email,
      });
      ...
      try {
        ...
        logger.success('ADMIN', `Welcome email sent to new user: ${customer.email}`);
      } catch (err) {
        logger.warn('ADMIN', `Welcome email failed: ${err.message}`, { error: err });
      }
    }
    ...
    if (existingUser) {
      ...
      try {
        if (existingUser.email || customer.email) {
          ...
          logger.success('ADMIN', `Email notification sent to: ${existingUser.email || customer.email}`);
        }
      } catch (e) {
        logger.warn('ADMIN', `Failed to send email notification: ${e.message}`, { error: e });
      }
    } else {
      logger.info('ADMIN', `No registered user found for phone: ${normPhone}`);
      ...
      try {
        ...
        logger.success('ADMIN', `SMS notification sent to unregistered user: ${normPhone}`);
      } catch (e) {
        logger.warn('ADMIN', `Failed to send SMS notification to unregistered user: ${e.message}`, { error: e });
      }
    }
