...
router.post('/create-order', paymentLimiter, authUser, requireUser, async (req, res) => {
  try {
    ...
    logger.info('RAZORPAY', 'Creating Razorpay order', { amount, currency, receipt });
    ...
    if (!order) {
      logger.error('RAZORPAY', 'Failed to create Razorpay order');
      return res.status(500).json({ error: 'Could not create Razorpay order' });
    }

    logger.success('RAZORPAY', `Razorpay order created: ${order.id}`, { rzpOrderId: order.id });
    res.json(order);
  } catch (error) {
    logger.error('RAZORPAY', `Create order failed: ${error.message}`, { error });
...
router.post('/verify', paymentLimiter, async (req, res) => {
  try {
    ...
    logger.info('RAZORPAY', 'Verifying Razorpay payment signature', {
      razorpay_order_id,
      razorpay_payment_id,
      order_id,
    });
    ...
    if (!razorpay_order_id || !razorpay_payment_id || !razorpay_signature) {
      logger.warn('RAZORPAY', 'Missing payment verification fields', {
        razorpay_order_id,
        razorpay_payment_id,
      });
    ...
    if (order_id) {
      ...
      if (order) {
        ...
        logger.success('RAZORPAY', `Payment verified and updated for order NC-${order.orderNo}`, {
          orderId: order._id,
          razorpay_order_id,
          razorpay_payment_id,
        });
        ...
        logger.warn('RAZORPAY', `Order not found during verification: ${order_id}`);
      }
    }
    ...
    logger.error('RAZORPAY', `Payment verification failed: ${error.message}`, { error });
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/refund', authAdmin, async (req, res) => {
  try {
    ...
    logger.info('RAZORPAY', `Processing refund for payment: ${payment_id}`, {
      payment_id,
      amount,
      orderId,
      reason,
    ...
    if (orderId) {
      ...
      if (order) {
        ...
        logger.success('RAZORPAY', `Refunded payment for order NC-${order.orderNo}`, {
          orderId: order._id,
          refundId: refund.id,
          amount: refund.amount,
        });
      } else {
        logger.warn('RAZORPAY', `Order not found during refund updates: ${orderId}`);
      }
    }

    res.json({ refund });
  } catch (error) {
    logger.error('RAZORPAY', `Refund process failed: ${error.message}`, { error });
    res.status(500).json({ error: error.message });
  }
});
