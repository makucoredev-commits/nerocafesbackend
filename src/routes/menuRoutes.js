...
router.get('/', async (req, res) => {
  try {
    ...
    const MenuItemModel = await req.getBranchModel('MenuItem');
    const allItems = await MenuItemModel.find(q, projection)
    ...
    // Always keep all menu items visible, labeled with their real-time available flag
...
router.get('/admin/all', authAdmin, requireBranchAccess, async (req, res) => {
  try {
    const { ShopSettings } = await import('../models/ShopSettings.js');
    const settings = await ShopSettings.findOne();
    const MenuItemModel = await req.getBranchModel('MenuItem');
...
router.post('/', authAdmin, requireBranchAccess, async (req, res) => {
  try {
    const {
      name,
      price,
...
router.patch('/:id', authAdmin, async (req, res) => {
  try {
    ...
    const MenuItemModel = await req.getBranchModel('MenuItem');
    const item = await MenuItemModel.findByIdAndUpdate(req.params.id, updates, { new: true, runValidators: true });
...
router.put('/:id', authAdmin, requireBranchAccess, async (req, res) => {
  try {
    const {
      name,
      price,
...
router.delete('/:id', authAdmin, requireBranchAccess, async (req, res) => {
  try {
    const MenuItemModel = await req.getBranchModel('MenuItem');
...
const handleMenuImageUpload = [
  ...
  (req, res) => {
    try {
      ...
      res.json({ imageUrl, url: imageUrl, success: true });
    } catch (e) {
...
router.post('/upload-image', ...handleMenuImageUpload);
router.post('/admin/upload', ...handleMenuImageUpload);
