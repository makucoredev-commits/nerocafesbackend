...
router.get('/', async (req, res) => {
  try {
    ...
    for (const item of allItems) {
      const maxQty = await getMenuItemMaxQuantity(item);
      const isAvailable = item.available !== false && (item.autoInventory ? maxQty > 0 : true);
      enrichedItems.push({
        ...item,
  ...
  try {
    ...
    for (const item of items) {
      ...
      normalizedItems.push({
        ...item,
        maxAvailableQuantity: maxQty,
        available: item.available !== false && (item.autoInventory ? maxQty > 0 : true),
      });
    }
...
router.patch('/:id', authAdmin, async (req, res) => {
  try {
    ...
    console.error('Menu item patch error:', e);
    res.status(400).json({ error: e.message });
  }
});
