...
// All routes require admin authentication and branch access verification
...
router.get('/queue', async (req, res) => {
  try {
    ...
    const rawQueue = await etaEngine.getKitchenQueue();
    // Strict branch isolation for kitchen queue
    const queue = rawQueue.filter(item => {
      const bId = item.order?.branchId || 'branch_001';
      return req.admin?.role === 'SUPER_ADMIN' && req.query.allBranches === 'true' ? true : bId === req.branchId;
    });
    res.json({ queue });
  } catch (e) {
...
router.patch('/orders/:orderId/status', async (req, res) => {
  try {
    ...
    // Strict cross-branch order isolation
    if (req.admin.role !== 'SUPER_ADMIN' && order.branchId && order.branchId !== req.branchId) {
...
router.get('/stats', async (req, res) => {
  try {
    const etaEngine = req.app.get('etaEngine');
    const snapshot = await etaEngine.getKitchenSnapshot(req.branchId);
    const stats = {
      queueSize: snapshot.currentQueue,
...
router.get('/workload', async (req, res) => {
  try {
    const etaEngine = req.app.get('etaEngine');
    const snapshot = await etaEngine.getKitchenSnapshot(req.branchId);
    const todayMetrics = await etaEngine.getTodayMetrics(req.branchId);
