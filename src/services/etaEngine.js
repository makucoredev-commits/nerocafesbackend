...
class ETAEngine {
  ...
  async initialize() {
    ...
    logger.success('ETA_ENGINE', `ETA Engine initialized with ${activeOrders.length} active orders`, { count: activeOrders.length });
  }
  ...
  async calculateOrderETA(order) {
    ...
    if (order.branchId) {
      activeOrdersFilter.branchId = order.branchId;
    }
    const activeOrders = await Order.find(activeOrdersFilter).sort({ createdAt: 1 });
  ...
  async calculateQueueWaitTime(newOrder) {
    ...
    if (newOrder.branchId) {
      activeOrdersFilter.branchId = newOrder.branchId;
    }
    const activeOrders = await Order.find(activeOrdersFilter).sort({ createdAt: 1 });
  ...
  async recalculateETA(orderId, trigger = 'manual') {
    ...
    logger.info('ETA_ENGINE', `Recalculated ETA for Order NC-${order.orderNo}: ${etaData.totalETA}min (trigger: ${trigger})`, {
      orderId: order._id,
      orderNo: order.orderNo,
      totalETA: etaData.totalETA,
      trigger,
    });
   ...
   */
  async recalculateAffectedOrders(excludeOrderId = null, branchId = null) {
    const filter = {
    ...
    const activeOrders = await Order.find(filter).sort({ createdAt: 1 });

    for (const order of activeOrders) {
      await this.recalculateETA(order._id.toString(), 'batch');
    }

    logger.info('ETA_ENGINE', `Recalculated ${activeOrders.length} affected orders in batch (branch: ${branchId || 'all'})`);
  }

  /**
   * Get queue position for an order (branch-scoped)
   */
  async getQueuePosition(order) {
    ...
    if (order.branchId) {
      filter.branchId = order.branchId;
    }

    const position = await Order.countDocuments(filter);
   ...
   */
  async getKitchenSnapshot(branchId = null) {
    const filter = {
      status: { $in: LIVE_ORDER_STATUSES },
      cancelledAt: null
    };
   ...
   */
  async getTodayMetrics(branchId = null) {
    ...
    if (branchId) {
      completedFilter.branchId = branchId;
      pendingFilter.branchId = branchId;
    }

    // Today's completed orders (revenue)
    const completedOrders = await Order.find(completedFilter);
  ...
  async handleOrderStatusChange(orderId, newStatus, oldStatus) {
    ...
    await this.recalculateAffectedOrders(orderId, order.branchId);

    // Deduplicate status notifications to make sure we emit exactly ONCE per status transition
    const deduplicatedOrder = await Order.findOneAndUpdate(
      { _id: orderId, notifiedStatuses: { $ne: newStatus } },
      { $addToSet: { notifiedStatuses: newStatus } },
      { new: true }
    );

    if (deduplicatedOrder) {
      // Emit status update
      this.emitStatusUpdate(order, newStatus, oldStatus);
    } else {
      logger.info('ETA_ENGINE', `Skipped duplicate notification emit for Order NC-${order.orderNo} to status ${newStatus}`);
    }
  }
  ...
  emitStatusUpdate(order, newStatus, oldStatus) {
    ...
    // Emit to order-specific room (customer tracking page) - only emit order:status to prevent duplicates
    this.io.to(`order:${order._id.toString()}`).emit('order:status', updateData);
    
    // Emit to customer room - only emit order:status to prevent duplicates
    this.io.to(customerRoom).emit('order:status', updateData);
    
    // Emit to user room (customer's other devices) - only emit order:status to prevent duplicates
    if (order.userId) {
      this.io.to(`user:${order.userId.toString()}`).emit('order:status', updateData);
    }
    
    // Emit to admin/kitchen rooms - keep order:status:update for admin compatibility
