...
// Determine and force correct process.env.NODE_ENV if not explicitly defined by environment.
// - If .env exists, we are running in production mode.
// - If .env.dev exists (and no .env), we are running in development mode.
// - Otherwise, default to production mode (safe fallback for cloud environments).
